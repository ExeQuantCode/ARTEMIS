...
module artemis__geom_rw
  ...
  type :: basis_type
   ...
   contains
     ...
     !! Procedure to normalise the basis.
...
!###############################################################################
  subroutine change_lattice(this, lattice)
    !! Change the lattice of the basis.
    !!
    !! This transforms the basis to a new lattice.
    implicit none
...
!###############################################################################
  subroutine normalise( &
    ...
    !! Normalise the basis to between 0 and 1.
    implicit none
    ...
    !! Optional. The ceiling value for normalisation.
    logical, intent(in), optional :: floor_coords
    !! Optional. Whether to floor the coordinates.
    logical, intent(in), optional :: round_coords
    !! Optional. Whether to round the coordinates.
    real(real32), intent(in), optional :: zero_round
    !! Optional. The value to set coordinates to if they are less than tol.

    ! Local variables
    integer :: is, ia, j
    !! Loop index.
    real(real32) :: ceil_val_, floor_val, tol
    !! The ceiling value, floor value, and tolerance.
    logical :: floor_coords_, round_coords_
    !! Boolean whether to floor and round the coordinates.
