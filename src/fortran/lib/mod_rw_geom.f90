...
module rw_geom
  ...
  double precision, dimension(3,3) :: lattice
...
!!!#############################################################################
 subroutine VASP_geom_read(UNIT,length)
   ...
   double precision, dimension(3,3) :: reclat
...
!!!-----------------------------------------------------------------------------
   if(basis%lcart)then
      reclat=transpose(LUinv(lattice))
...
!!!#############################################################################
 subroutine VASP_geom_write(UNIT,lat_write,bas_write,lcart)
   implicit none
   integer :: i,j,UNIT
   double precision, dimension(3,3) :: lat_write
...
!!!#############################################################################
 subroutine QE_geom_read(UNIT,length)
   ...
   integer, intent(in), optional :: length
   double precision, dimension(3) :: tmpvec
   double precision, dimension(3,3) :: reclat
...
!!!#############################################################################
 subroutine CASTEP_geom_read(UNIT,length)
   ...
   logical :: labc
   integer, dimension(1000) :: tmp_natom
   double precision, dimension(3) :: abc,angle,dvtmp1
   double precision, dimension(3,3) :: reclat
   character(len=5), dimension(1000) :: tmp_spec
   integer, intent(in), optional :: length
...
!!!#############################################################################
 subroutine CASTEP_geom_write(UNIT,lat_write,bas_write,labc,lcart)
   implicit none
   integer :: i,j,UNIT
   double precision, dimension(3) :: abc,angle
   double precision, dimension(3,3) :: lat_write
   type(bas_type) :: bas_write
...
!!!#############################################################################
 subroutine XYZ_geom_read(UNIT,length)
   ...
   double precision, allocatable, dimension(:,:,:) :: tmp_bas
   character(len=5) :: ctmp
...
!!!#############################################################################
 function convert_bas(inbas,latconv) result(outbas)
   ...
   double precision, dimension(3,3), intent(in) :: latconv
...
!!!#############################################################################
 function convert_abc_to_lat(abc,angle,radians) result(out_lat)
   ...
   implicit none
   double precision, dimension(3) :: in_angle
   double precision, dimension(3,3) :: out_lat

   double precision, dimension(3), intent(in) :: abc,angle
...
!      in_angle=angle*pi/180.D0 ! this looks wrong, check it
   ...
   out_lat(3,3) = sqrt(abc(3)**2.D0 - out_lat(3,1)**2.D0 - out_lat(3,2)**2.D0)
...
!!!#############################################################################
 function convert_lat_to_abc(in_lat,radians) result(abc_angle)
   ...
   implicit none
   integer :: i
   double precision, dimension(2,3) :: abc_angle

   double precision, dimension(3,3), intent(in) :: in_lat
...
!!!#############################################################################
 subroutine clone_bas(inbas,outbas,inlat,outlat,trans_dim)
   ...
   double precision, dimension(3,3), optional :: inlat,outlat
